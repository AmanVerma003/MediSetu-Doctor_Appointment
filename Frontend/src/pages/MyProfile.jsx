...
function MyProfile() {
  ...
  const updateProfile = async () =>{
    try{
      ...
      const {data} = await axios.patch(`${backendUrl}/api/user/update-profile`,
        formData,  {
        headers: {
      ...
      if (error.response?.data?.message) {
        ...
        toast.error("Failed to update userProfile. Please try again.");
      }
    }
  }
        ...
        {isEdit ? (
          <label htmlFor="image">
            <div className="inline-block relative cursor-pointer">
              <img
                className="w-26 rounded opacity-75"
          ...
          <img className="w-26 rounded" src={userData.image} />
        )}
        ...
        <div className="mt-10">
          {isEdit ? (
            <button
              ...
              className="border border-blue-400 px-8 py-2 rounded-full hover:bg-blue-400 hover:text-white transition-all cursor-pointer"
            >
              Save information
            </button>
          ) : (
